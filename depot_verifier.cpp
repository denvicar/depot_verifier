...
int main(int argc, char* argv[]) 
{
    ...
    if (!filesystem::exists(acf_dir) || !filesystem::exists(manifest_dir) || !filesystem::exists(game_dir)) {
        cerr << "Check depot_verifier.ini, one of the specified paths does not exist\n";
        return 1;
    }
    ...
    for (int i = 1; i < argc; ++i) {
        for (const filesystem::directory_entry& entry : filesystem::directory_iterator{ acf_dir }) {
            if (entry.path().string().find(argv[i]) != string::npos) {
                ...
                std::cout << "appid: \033[34m" << argv[i] << "\033[0m"
                    ...
                    << "\nmanifests: \033[34m" << info.manifests.size() << "\033[0m\n";

                int missing=0, wrong=0, ok=0;

                filesystem::path game_main_dir = game_dir/(info.installdir);

                for (const filesystem::directory_entry& game_entry : filesystem::recursive_directory_iterator{ game_main_dir }) {
                ...
                for (auto it = info.manifests.rbegin(); it != info.manifests.rend(); ++it) {
                    cout << "manifest: " << *it << '\n';
                    ...
                    while (dm.has_next()) {
                        ...
                        if (find(processed_files.begin(), processed_files.end(), df.filename())!=processed_files.end()) continue;
                        cout << "validating " << df.filename() << " " << processed_files.size()+1 << '/' << game_files.size() << ' ';
                        ...
                        else if (auto sha = get_hash_content(ctx, df_path); sha != get_hex_string(df.sha_content())) {
                ...
                summary.insert(make_pair(info.name, vector<int>{static_cast<int>(processed_files.size()), missing, wrong, ok}));
                ...
                if (processed_files.size() != game_files.size()) {
                    ...
                    for (const auto& game_file : game_files) {
                        if (find_if(processed_files.begin(),processed_files.end(), [game_file](string name){return name.find(game_file.filename().string())!=string::npos;}) == processed_files.end())
