...
acf_info get_acf_info(path p) {
    ...
    while (getline(f, line)) {
        ...
        if (auto p = line.find(ACF_NAME); p != string::npos) {
            ...
            info.installdir = line.substr(info_start, line.find_last_of("\"") - info_start);
        }
        ...
        if (manifest_part) {
            if (line[0] == '}') {
                if (bracket_count == 1) manifest_part = false;
                else bracket_count++;
            }
            if (auto p = line.find(MANIFEST_START); p != string::npos) {
                ...
                string manifestId = line.substr(info_start, line.find_last_of("\"") - info_start);
                info.manifests.push_back(manifest_prefix + "_" + manifestId + ".manifest");
                bracket_count = 0;
            }
