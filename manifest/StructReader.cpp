...
std::size_t StructReader::size() const {
	return data.size();
}

std::vector<uint8_t> StructReader::read(const int n = 1) {
	this->offset += n;
	return std::vector<uint8_t>(&(this->data[this->offset - n]), &(this->data[this->offset]));
}
...
std::vector<uint32_t> StructReader::unpack(const std::string& format_text) {
	...
	for (int i = 0; i < number_of_elements; ++i) {
		res.push_back( get_unsignedint( vector<uint8_t>(data.begin()+offset,data.begin() + (offset + sizeof(uint32_t)))));
